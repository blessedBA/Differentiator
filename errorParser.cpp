...
void updateErrorMask (errorLog* log, code_error_t code)
{
    ...
    log->global_mask_error |= (1ULL << code);
...
void addErrorContext (errorLog* log, const char* file_name, const char* func_name, int line)
{
    ...
    if (log->entries->ctx_size >= log->entries->ctx_capacity) grow_contexts(log->entries);

    errorContext *curr_context = &log->entries->contexts[log->entries->ctx_size++];
    ...
    strncpy(curr_context->time_date, time_buf, sizeof(time_buf));

    FILE* err_log_file = log->log_file;

    fprintf(err_log_file, COLOR_BRED "ERROR CODE %d | %s | %s:%d in %s() at %s\n" COLOR_RESET,
                log->entries->code, log->entries->description, file_name,
...
void printErrors (const errorLog* log)
{
    ...
    FILE* log_file = log->log_file;

    fprintf(log_file, "ERROR CODE = %llu\n", log->global_mask_error);

    for (size_t index = 0; index < log->size; index++)
    {
        if (log->entries[index].has_error)
        {
            fprintf(log_file, "\n=== ERROR %zu ===\n", index);
            fprintf(log_file, "Code: %d\nDescription: %s\nContexts: %zu\n",
                        log->entries->code, log->entries->description, log->entries->ctx_size);

            for (size_t sec_index = 0; sec_index < log->entries->ctx_size; sec_index++)
            {
                const errorContext* curr_context = &log->entries[index].contexts[sec_index];
                fprintf(log_file,
                    "  [%zu] %s:%d in %s() at %ld\n",
                    sec_index, curr_context->file, curr_context->line,
                    curr_context->function, curr_context->timestamp);
            }
        }
    }
